...
A separate, fully deterministic gate decides the merge, in a job of its own, from conditions it re-derives itself at merge time — including that the author is Dependabot, the PR is open, and every changed path is a dependency manifest/lockfile for one of your configured ecosystems (__WHITELIST_HUMAN__). It decides in one of two ways:
