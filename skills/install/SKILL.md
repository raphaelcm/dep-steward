...
- **The Claude Code GitHub App:** there is no API that reports this reliably. Treat it as required and tell the user to confirm it at <https://github.com/apps/claude> → Configure → add this repo. Without it, every review and autofix run fails with "Claude Code is not installed on this repository". Autofix also pushes its fixes as this App, which is what makes CI run on them.
