...
- **Autofix's push token exists only for a fix that will be pushed, and can only push.** With dep-steward's GitHub App configured, the autofix job mints an installation token after the bounds check accepts a fix — never before it, and never on a discard or escalate path — scoped to `contents: write` on this repository alone and revoked when the job ends. The agent never sees it or the App's key: only the mint step reads the key, and only the push step receives the token (`test/autofix-push.test.mjs` checks both by running the job). The App itself should hold Contents: read and write and nothing else — never Workflows.
...
- **CI on an autofixed PR runs with your Actions secrets, once you add the App.** A CI run Dependabot starts gets the Dependabot secret store and a read-only token. A run the App's push starts is an ordinary PR run, like one after a person pushes to the PR, so it gets your Actions secrets and whatever your CI hands to code under test. The code it runs is the dependency bump plus a few lines the fixer wrote. Anything in your CI that releases secrets based on PR state the agents can change reaches that run with no person in between: both the reviewer and the fixer can add labels, so a label that opts a PR into secret-bearing jobs is the case to check before adding the App.
- **The App's private key is a long-lived repository secret.** Anyone who can run a workflow with access to your Actions secrets could mint tokens with the App's permissions while the key is valid. Keep the App to Contents: read and write, install it on this repository only, and rotate the key if you suspect exposure.
