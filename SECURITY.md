...
- **Nothing an agent writes is ever run.** Both agents can write files (the reviewer its comment draft, the fixer the source it fixes). Their Write and Edit rules name no path, and Claude Code matches such a rule everywhere, so they can write any file but its protected ones (`.git`, `.claude`, shell rc files): in the checkout, which holds this pipeline's scripts, and outside it. A script run after an agent starts could therefore be the agent's rewrite, and it would run with what its caller holds: in the review's lint hook, the Claude session's environment (the Claude App token, `GITHUB_TOKEN`, `CLAUDE_CODE_OAUTH_TOKEN`); in a later step, `GITHUB_TOKEN` and the job's OIDC token. So every such script (the lint hook, the deliverable assertion's gate and lint, autofix's bounds check) runs from a copy taken before the agent starts. The copy is read-only, and the agents have no shell to change that. Nothing else they could write outside the checkout steers a later step either: checkout persists no credential (it would be a file the repo's git config includes, where git config such as `core.fsmonitor` runs programs), and the push step's `curl` reads no `~/.curlrc` (a proxy named there would receive its tokens). Autofix takes its copies and its prompt from the commit its workflow runs from, the default branch's, as the gate does. `test/agent-writes.test.mjs` and `test/autofix-push.test.mjs` run both jobs with an agent that rewrites every script it can reach.
